...
import De.Hpi.DesisAll.DesisMultipleKeys.Dao.*;
...
public class OptimizerSingleKey implements Runnable{
...
//    private long latencyCounter;
    ...
    public OptimizerSingleKey(Configuration conf, ConcurrentLinkedQueue<WindowCollection> intermediateResultQueue,
...
//                            if (tupleCounter - task.getProcessTime() > task.query.getBatchSize()) {
                        if (tupleCounter - task.getProcessTime() >= conf.localBatchSize) {
                            //for the state
//                            localisEventHere.stateList[task.getTaskId()] = conf.EVENTENDANDSTART;
                            localisEventHere.setCreateNewWindow(true);
                            localisEventHere.setFinishWindow(true);
